...
parser.add_argument(
    "add_epic_var", metavar="add_epic_var", type=str, help="name of new epic variable"
)
...
EPIC_VARS_dict = ConfigParserLocal.get_config(
    parent_dir + "/config_files/epickey.json", ftype="json"
)
...
for count, ncfile in enumerate(
    ...
    try:
        ...
        newvar = nchandle.createVariable(
            EPIC_VARS_dict[epic_var_ind]["EPIC_KEY"],
            "f4",
            ("time", "dep", "lat", "lon"),
            fill_value=1e35,
        )
        ...
        if not "History" in global_atts.keys():
            ...
            nchandle.setncattr(
                "History",
                "{histtime:%B %d, %Y %H:%M} UTC {variable} added".format(
                    histtime=histtime, variable=args.add_epic_var
                ),
            )
        else:
            histtime = datetime.datetime.utcnow()

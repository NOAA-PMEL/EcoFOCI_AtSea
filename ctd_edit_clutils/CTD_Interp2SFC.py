...
parser = argparse.ArgumentParser(description='Interpolate to SFC from chosen depth for CTD files')
parser.add_argument('ctd_file', 
    metavar='ctd_file', 
    type=str, 
    help='full path to ctd file')
parser.add_argument('depth', 
    metavar='depth', 
    type=int, 
    help='depth to interpolate to sfc from')
...
for variable in vars_dic.keys():
    if not variable in ['time','time2','dep','depth','P_1','D_3',
                        'lat','latitude','lon','longitude']:  
        nchandle.variables[variable][0,depth_ind,0,0] = nchandle.variables[variable][0,args.depth,0,0]

...
parser = argparse.ArgumentParser(description='Merge and archive oxygen csv data and bottle data')
...
parser.add_argument('oxypath', 
    metavar='oxypath', 
    type=str, 
    help='full path to oxygen csv file')
...
if args.cf:
    ...
    for i,cast in enumerate(gb.groups):
        ...
        try:
            ...
            print("O2 field skipped as no column match to \"O2 uM/l\" - remove comma if in string")
        ...
        data_dic['time_temp'] =  pd.to_datetime(data_dic['time'], format='%Y%m%d %H:%M:%S')
        data_dic['time'] = [date2num(x[1],'hours since 1900-01-01T00:00:00Z') for x in enumerate(data_dic['time_temp'] )]
    ...
    for i,cast in enumerate(gb.groups):
        ...
        ncinstance.sbeglobal_atts(raw_data_file=args.oxypath.split('/')[-1],
                                    CruiseID=cruise,
                                    Cast=cast)

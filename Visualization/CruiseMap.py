...
def read_data(db, cursor, table, cruiseID, dbvar="CruiseID", latvarname="LatitudeDeg"):
    ...
    sql = (
        "SELECT * from `{0}` WHERE `{1}`='{2}' AND `{3}` NOT LIKE '' AND `{3}` NOT LIKE 'NOT DEPLOYED' AND `{3}` NOT LIKE '-99'"
    ).format(table, dbvar, cruiseID, latvarname)
    print(sql)
...
def sqldate2GEdate(castdate, casttime):
    ...
    try:
        outstr = datetime.datetime.strptime(
            (castdate + " " + casttime), "%Y-%b-%d %H:%M"
...
def cartopy_plot(
    ...
    if args.labels:
        ...
        projection = ccrs.LambertConformal(
            central_longitude=-160.0, standard_parallels=(45, 55)
        )

    transformation = ccrs.PlateCarree()

    # Cruise Data
    x_cast, y_cast = -1.0 * cast_lon, cast_lat
    x_moor, y_moor = -1.0 * mooring_lon, mooring_lat
    xr_moor, yr_moor = -1.0 * rmooring_lon, rmooring_lat
    xa_moor, ya_moor = -1.0 * data_argo_lon, data_argo_lat
    xd_moor, yd_moor = -1.0 * data_drifters_lon, data_drifters_lat
    ...
    land_50m = cfeature.NaturalEarthFeature(
        "physical", "land", "50m", edgecolor="face", facecolor="1.0"
    )

    ax.contourf(
        bathy_sub.longitude,
        bathy_sub.latitude,
        bathy_sub[z_var_name],
    ...
    # plot points
    ax.scatter(x_cast, y_cast, 10, marker="+", color="r", transform=transformation)
    ax.scatter(
        x_moor,
        y_moor,
        12,
    ...
    for i, v in enumerate(x_cast):
        ax.text(
            x_cast[i], y_cast[i], cast_name[i], fontsize=3, transform=transformation
        )
    # add station labels
    for i, v in enumerate(x_moor):
        ax.text(
            x_moor[i], y_moor[i], mooring_name[i], fontsize=3, transform=transformation
        )
    ...
    if filetype in ["png"]:
        plt.savefig(
            "images/" + cruiseID + "/" + cruiseID + "_map.png",
            bbox_inches="tight",
            dpi=(300),
        )
    if filetype in ["svg"]:
        plt.savefig(
            "images/" + cruiseID + "/" + cruiseID + "_map.svg",
            bbox_inches="tight",
            dpi=(300),
        )
    plt.close()
...
def kml_description_box(kml, data):
    ...
    kml = kml + ("<tr><td><strong>CTD:</strong> {0}</td>" "<td>{1}</td></tr>\n").format(
        data["ConsecutiveCastNo"], data["StationNameID"]
    )
    kml = kml + (
        "<tr><td><strong>Time (GMT):</strong> {0}-{1}-{2} {3}</td>" "<td></td></tr>\n"
    ).format(data["GMTYear"], data["GMTMonth"], data["GMTDay"], data["GMTTime"])
    kml = kml + (
        "<tr><td><strong>Lat (N):</strong> {0}' {1}''</td><td></td>"
        "<td></td><td></td></tr>\n"
    ).format(data["LatitudeDeg"], data["LatitudeMin"])
    kml = kml + (
        "<tr><td><strong>Lon (W):</strong> {0}' {1}''</td><td></td>"
        "<td></td><td></td></tr>\n"
    ...
    kml = kml + (
        "<tr><td><strong>Bottom Depth:</strong> {0}</td><td></td></tr>\n"
    ).format(data["BottomDepth"])
    kml = kml + ("<tr><td><strong>Max Depth:</strong> {0}</td><td></td></tr>\n").format(
        data["MaxDepth"]
    )
    kml = kml + (
        "<tr><td><strong>Estimated Niskin ID:</strong></td><td>{0}</td><td></td></tr>\n"
    ).format(data["NutrientBtlNiskinNo"])
    kml = kml + (
        "<tr><td><strong>Estimated Oxygen Niskin ID:</strong></td><td>{0}</td><td></td></tr>\n"
    ).format(data["OxygenBtlNiskinNo"])
    kml = kml + (
        "<tr><td><strong>Estimated Salinity Niskin ID:</strong></td><td>{0}</td><td></td></tr>\n"
    ).format(data["SalinityBtlNiskinNo"])
    kml = kml + (
        "<tr><td><strong>Estimated Chlorophyll Niskin ID:</strong></td><td>{0}</td><td></td></tr>\n"
    ...
    kml = kml + (
        "<tr><td><strong>T:</strong> {0}</td><td><strong>P:</strong> {1}</td>"
    ...
    kml = kml + (
        "<tr><td><strong>Instrument SN:</strong> {0}</td><td></td></tr>\n"
    ).format(data["InstrumentSerialNos"])
    kml = kml + (
        "<tr><td><strong>Notes:</strong> {0}</td><td></td><td></td></tr>\n"
    ).format(data["Notes"])
    kml = kml + ("</tbody></table>\n" "</description>\n")
...
parser = argparse.ArgumentParser(
    description="KML and PNG maps from Cruise Log Database"
)
parser.add_argument(
    "CruiseID",
    metavar="CruiseID",
...
parser.add_argument(
    "-geojson", "--geojson", action="store_true", help=("Make GeoJSON Map")
)
parser.add_argument("-csv", "--csv", action="store_true", help=("Output .csv file"))
parser.add_argument(
    "-host", "--host", type=str, default="localhost", help=("local or pavlof")
)
...
parser.add_argument(
    ...
    type=str,
    default="etopo1",
    help="etopo1,smith_sandwell,etop5(not available yet)",
)
parser.add_argument(
    "-labels", "--labels", action="store_true", help="turn on lat/lon labels"
)
####
...
if (
    ".txt" in cruiseID_input
):  # assume file hasone cruise per line with tab if Leg is identified
...
for cruiseID in cruiseID_input:
    ...
    db_config = ConfigParserLocal.get_config(
        "../../EcoFOCI_Config/EcoFOCI_AtSea/db_config_cruises.yaml"
    )

    # get db meta information for mooring
    ### connect to DB
    (db, cursor) = connect_to_DB(
        host=db_config["systems"][args.host]["host"],
    ...
    # mooring db
    db_config = ConfigParserLocal.get_config(
        "../../EcoFOCI_Config/EcoFOCI_AtSea/db_config_mooring.yaml"
    )
    ...
    data_mooring = read_data(
        db, cursor, mtable, cruiseID, dbvar="CruiseNumber", latvarname="Latitude"
    )
    if not data_mooring.keys():
        cruiseIDt = cruiseID[:4] + "-" + cruiseID[4:]
        data_mooring = read_data(
            db, cursor, mtable, cruiseIDt, dbvar="CruiseNumber", latvarname="Latitude"
        )

    rdata_mooring = read_data(
        db, cursor, rtable, cruiseID, dbvar="CruiseNumber", latvarname="Latitude"
    )
    if not rdata_mooring.keys():
        cruiseIDt = cruiseID[:4] + "-" + cruiseID[4:]
        rdata_mooring = read_data(
            db, cursor, rtable, cruiseIDt, dbvar="CruiseNumber", latvarname="Latitude"
        )
    close_DB(db)
    ...
    # mooring db
    db_config = ConfigParserLocal.get_config(
        "../../EcoFOCI_Config/EcoFOCI_AtSea/db_config_drifters.yaml"
    )
    ...
    data_argo = read_data(
        db, cursor, argo_table, cruiseID, dbvar="CruiseID", latvarname="ReleaseLat"
    )
    if not data_argo.keys():
        cruiseIDt = cruiseID[:4] + "-" + cruiseID[4:]
        data_argo = read_data(
            db, cursor, argo_table, cruiseIDt, dbvar="CruiseID", latvarname="ReleaseLat"
        )

    data_drifters = read_data(
        db, cursor, drifter_table, cruiseID, dbvar="CruiseID", latvarname="ReleaseLat"
    )
    ...
    cast_lat = np.array(
        [
            float(data[a_ind]["LatitudeDeg"]) + float(data[a_ind]["LatitudeMin"]) / 60.0
            for a_ind in list(data.keys())
        ]
    )
    cast_lon = np.array(
        [
            float(data[a_ind]["LongitudeDeg"])
            + float(data[a_ind]["LongitudeMin"]) / 60.0
            for a_ind in list(data.keys())
        ]
    )
    cast_date = [
        str(data[a_ind]["GMTYear"])
        + "-"
        + data[a_ind]["GMTMonth"]
        + "-"
        + str(data[a_ind]["GMTDay"])
        for a_ind in list(data.keys())
    ]
    cast_time = [
        convert_timedelta(data[a_ind]["GMTTime"]) for a_ind in list(data.keys())
    ]
    ...
    # recovered moorings
    rmooring_name = [
        rdata_mooring[a_ind]["MooringID"] for a_ind in rdata_mooring.keys()
    ]
    try:
        rmooring_lat = np.array(
            [
                float(rdata_mooring[a_ind]["Latitude"].split()[0])
                + float(rdata_mooring[a_ind]["Latitude"].split()[1]) / 60.0
                for a_ind in rdata_mooring.keys()
            ]
        )
        rmooring_lon = np.array(
            [
                float(rdata_mooring[a_ind]["Longitude"].split()[0])
                + float(rdata_mooring[a_ind]["Longitude"].split()[1]) / 60.0
                for a_ind in rdata_mooring.keys()
            ]
        )
    except ValueError:  # or empty
    ...
    try:
        data_argo_lat = np.array(
            [
                float(data_argo[a_ind]["ReleaseLat"].split()[0])
                + float(data_argo[a_ind]["ReleaseLat"].split()[1]) / 60.0
                for a_ind in data_argo.keys()
            ]
        )
        data_argo_lon = np.array(
            [
                float(data_argo[a_ind]["ReleaseLon"].split()[0])
                + float(data_argo[a_ind]["ReleaseLon"].split()[1]) / 60.0
                for a_ind in data_argo.keys()
            ]
        )
    except ValueError:  # or empty
    ...
    # argos drifters
    data_drifters_name = [
        data_drifters[a_ind]["ArgosNumber"] for a_ind in data_drifters.keys()
    ]
    try:
        data_drifters_lat = np.array(
            [
                float(data_drifters[a_ind]["ReleaseLat"].split()[0])
                + float(data_drifters[a_ind]["ReleaseLat"].split()[1]) / 60.0
                for a_ind in data_drifters.keys()
            ]
        )
        data_drifters_lon = np.array(
            [
                float(data_drifters[a_ind]["ReleaseLon"].split()[0])
                + float(data_drifters[a_ind]["ReleaseLon"].split()[1]) / 60.0
                for a_ind in data_drifters.keys()
            ]
        )
    except ValueError:  # or empty
    ...
    if args.kml:
        ...
        for ind, value in enumerate(list(data.keys())):
            ...
            kml = kml + (
                ...
                cast_lat[ind],
                -1 * cast_lon[ind],
                sqldate2GEdate(cast_date[ind], cast_time[ind]),
            )
    ...
    if args.geojson:
        ...
        for ind, value in enumerate(list(data.keys())):
            geojson_Features = geojson_Features + (
                ...
                ind,
                data[value]["Project"],
                data[value]["ConsecutiveCastNo"],
                data[value]["StationNameID"],
                data[value]["GMTYear"],
                data[value]["GMTMonth"],
                data[value]["GMTDay"],
                data[value]["GMTTime"],
